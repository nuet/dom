...
<div class="u_SheJiS_DaTing u_fl">
        ...
        <div class="u_SheJiS_DaTing_xz">
              <div class="u_SheJiS_DaTing_xz_t"><b><i>> 平面项目大厅</i></b></div>
              <div class="u_SheJiS_DaTing_xz_Tiaoj">
                <div class="u_SheJiS_DaTing_xz_Tiaoj_t">
               
                  <b>平面项目搜索></b> <a href="">重置筛选条件</a>
                </div>
                    ...
                    <div class="u_SheJiS_DaTing_xz_Tiaoj_sea_1 u_SheJiS_DaTing_xz_Tiaoj_sea_1_bg">
                      <b>套餐级别：</b> 
                          <ul>
                            ...
                            <li><a href="?t=4&n=<?php echo $s;?>&order=<?php echo $order;?>">基础型</a></li>
                            <li><a href="?t=5&n=<?php echo $s;?>&order=<?php echo $order;?>">标准型</a></li>
                            <li><a href="?t=6&n=<?php echo $s;?>&order=<?php echo $order;?>">高级型</a></li>
                          </ul>
                    </div>
           ...
           <?php if(is_array($trades)) { foreach($trades as $i => $r) { ?>
            ...
            <div class="u_SheJiS_DaTing_List_cont">
              ...
              <div class="u_SheJiS_DaTing_List_cont_xmzt">
                    ...
                    <p><a href="design.php?itemid=<?php echo $r['itemid'];?>&action=update&step=upload_5&forward=<?php echo $MODULE['2']['linkurl'];?>">订单详情</a></p>
              </div>
       ...
       <input type="button" value="等待中" class="gaoliangG">
                ...
                <?php } else if($r['status']==2) { ?>
                 <input type="submit" value="提交首页" class="u_btn" onclick="Go('design.php?itemid=<?php echo $r['itemid'];?>&action=update&step=index&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
              
                 <?php } else if($r['status']==3 ) { ?>
                 <input type="submit" value="提交首页" class="u_btn" onclick="Go('design.php?itemid=<?php echo $r['itemid'];?>&action=update&step=index&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
                 <?php } else if($r['status']==5 ) { ?>
                 <input type="submit" value="提交栏目" class="u_btn" onclick="Go('design.php?itemid=<?php echo $r['itemid'];?>&action=update&step=lm&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
                 <?php } else if($r['status']==6 || $rm['status']==7 ) { ?>
                   <input type="submit" value="提交PSD" class="u_btn" onclick="Go('design.php?itemid=<?php echo $r['itemid'];?>&action=update&step=design&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
                  <?php } else if($r['status']==8 ) { ?> 
                  <?php if($r['buyer_star']) { ?>
<input type="submit" value="评价详情" class="u_btn" onclick="Go('design.php?itemid=<?php echo $r['itemid'];?>&action=update&step=comment_detail&forward=<?php echo $forward;?>');"/>
<?php } else { ?>
<input type="submit" value="评 价" class="u_btn" onclick="Go('design.php?itemid=<?php echo $r['itemid'];?>&action=update&step=comment&forward=<?php echo $forward;?>');"/>
<?php } ?>
                 <?php } else { ?>
                     <input type="submit" value="等待中" class="gaoliangG">
                 <?php } ?>
