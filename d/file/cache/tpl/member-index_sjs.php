 ...
 <?php if(is_array($list)) { foreach($list as $i => $rm) { ?>
          <div class="u_rtwo_3 u_clear_d">
           <div class="u_rtwo_2_1 u_fl u_pt10">
            ...
            <div class="u_rtwo_2_2 u_fl u_pt10"><span><?php echo $rm['buyer_receive'];?>  </span></div>
            <div class="u_rtwo_2_3 u_fl u_pt10"><span><br><?php echo $rm['count'];?></span></div>
            <div class="u_rtwo_2_4 u_fl u_pt10"><span><br><?php echo $rm['status_zt'];?><br><?php if($rm['status']==0 || $rm['status']==1) { ?><span style="color:#ADADAD;">订单详情</span> <?php } else { ?><a href="project.php?itemid=<?php echo $rm['itemid'];?>&action=update&step=upload_5&forward=<?php echo $MODULE['2']['linkurl'];?>">订单详情</a><?php } ?>
</span></div>
            <div class="u_rtwo_2_5 u_fl u_pt10"><span>  <?php echo $rm['lefttime'];?>  </span></div>
       ...
       <input type="button" value="等待客户付款" class="gaoliangG">
                ...
                <?php } else if($rm['status']==2) { ?>          
                 ...
                 <?php } ?>
                 <?php } else if($rm['status']==6 || $rm['status']==7 ) { ?>
                  <?php if($rm['designer']==$_company) { ?>
                   <input type="submit" value="提交PSD" class="u_btn" onclick="Go('project.php?itemid=<?php echo $rm['itemid'];?>&action=update&step=design');"/>
                     <?php } else { ?>
                     <input type="submit" value="提交PSD" class="gaoliangG" />
                    <?php } ?>
                  
                  <?php } else if($rm['status']==8 ) { ?> 
                   <?php if($rm['designer']==$_company) { ?>
                  <?php if($rm['buyer_star']) { ?>
<input type="submit" value="评价详情" class="u_btn" onclick="Go('project.php?itemid=<?php echo $rm['itemid'];?>&action=update&step=comment_detail&forward=<?php echo $forward;?>');"/>
<?php } else { ?>
<input type="submit" value="评 价" class="u_btn" onclick="Go('project.php?itemid=<?php echo $rm['itemid'];?>&action=update&step=comment&forward=<?php echo $forward;?>');"/>
<?php } ?>
 <?php } else { ?>
                     <input type="submit" value="评 价" class="gaoliangG" />
                    <?php } ?>
                 <?php } else { ?>
...
<?php if(is_array($lists)) { foreach($lists as $i => $rn) { ?>
          <div class="u_rtwo_3 u_clear_d">
            ...
            <div class="u_rtwo_2_2 u_fl u_pt10"><span><?php echo $rn['buyer_receive'];?>  </span></div>
            <div class="u_rtwo_2_3 u_fl u_pt10"><span><br><?php echo $rn['count'];?></span></div>
            <div class="u_rtwo_2_4 u_fl u_pt10"><span><br><?php echo $rn['status_zt'];?><br><a href="design.php?itemid=<?php echo $rn['itemid'];?>&action=update&step=upload_5&forward=<?php echo $MODULE['2']['linkurl'];?>">订单详情</a></span></div>
            <div class="u_rtwo_2_5 u_fl u_pt10"><span>  <?php echo $rn['lefttime'];?>  </span></div>
            <div class="u_rtwo_2_6 u_fl u_pt10"><span>
                <?php if($rn['status']==0) { ?>
       <input type="button" value="等待中" class="u_btn">
                <?php } else if($rn['status']==1) { ?>
                <input type="submit" value="等待中" class="u_btn">
                <?php } else if($rn['status']==2) { ?>          
                 <input type="submit" value="提交首页" class="u_btn" onclick="Go('design.php?itemid=<?php echo $rn['itemid'];?>&action=update&step=index&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
                 <?php } else if($rn['status']==3 ) { ?>
                 <input type="submit" value="提交首页" class="u_btn" onclick="Go('design.php?itemid=<?php echo $rn['itemid'];?>&action=update&step=index&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
                 <?php } else if($rn['status']==4 ||$rn['status']==5 || $rn['status']==6 || $rn['status']==7 ) { ?>
                   <input type="submit" value="提交PSD" class="u_btn" onclick="Go('design.php?itemid=<?php echo $rn['itemid'];?>&action=update&step=design&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
