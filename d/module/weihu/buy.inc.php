...
if($submit) {
			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				if($v['express'] && in_array($v['express'], array(1,2,3))) {
					...
					if($express_id && $area_id) {
						$E = $db->get_one("SELECT * FROM {$DT_PRE}weihu_express WHERE itemid=$express_id");
						if($E && $E['items'] > 0) {
							$AREA = cache_read('area.php');
							$aid = $area_id;
							$ii = 0;
							do {
								$E = $db->get_one("SELECT * FROM {$DT_PRE}weihu_express WHERE parentid=$express_id AND areaid=$aid");
								if($E) {
				...
				$db->query("INSERT INTO {$DT_PRE}weihu_order (mallid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note,buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,buyer_receive,status,fee_name,fee,dmhf,mmcz,jscl,htsh,ymwh,sjwh,wzlr,cplr,endtime) VALUES ('$itemid','$_username','$t[username]','$title','$t[thumb]','$t[price]','$number','$amount','$DT_TIME','$DT_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile','$buyer_receive','$status','$fee_name','$fee','$t[dmhf]','$t[mmcz]','$t[jscl]','$t[htsh]','$t[ymwh]','$t[sjwh]','$t[wzlr]','$t[cplr]','$endtime')");
				$itemid = $db->insert_id();
				$db->query("REPLACE INTO {$DT_PRE}weihu_comment (itemid,mallid,buyer,seller) VALUES ($itemid,$k,'$_username','$t[username]')");
				$tmp = $db->get_one("SELECT mallid FROM {$DT_PRE}weihu_stat WHERE mallid=$k");
				if(!$tmp) $db->query("REPLACE INTO {$DT_PRE}weihu_stat (mallid,buyer,seller) VALUES ($k,'$_username','$t[username]')");
				//send message
				$touser = $t['username'];
				$_title = $title;
				$title = lang($L['trade_message_t6'], array($itemid));
				$url = $MODULE[2]['linkurl'].'weihu.php?itemid='.$itemid;
	...
	$do->set($cart);
	message($L['msg_buy_success'], $MODULE[2]['linkurl'].(($status == 1 && $i == 1 && $itemid) ? 'weihu.php?action=update&step=pay&itemid='.$itemid : 'weihu.php?action=order'));
} else {
	$tags = $_tags = array();
	$receive = '';
	if(is_array($itemid)) {
