...
require MD_ROOT.'/weihu.class.php';
$do = new weihu($moduleid);
$menus = array (
...
switch($action) {
	case 'add':
		if($submit) {
			...
			$EXP = array();
			$result = $db->query("SELECT * FROM {$DT_PRE}weihu_express WHERE username='$username' AND parentid=0 ORDER BY listorder ASC,itemid ASC LIMIT 100");
