...
$sj_all=$db->get_all("select * from d_mall_order_1 where company_id='$COM[username]' group by rid");
$sj_in=$db->get_one("SELECT count(*)as count FROM d_mall_order_1  where status=1 and level=1 and designer='$COM[company]'");
$sj_last=$db->get_one("SELECT count(*)as count FROM d_mall_order WHERE designer='$COM[company]' and status=8");
