	...
	switch($step) {
		case 'upload_1'://设计师上传第一次作品
			...
			case 'upload_5'://买家上传需求
				...
				if($submit) {
					...
					$upload_sql="update d_mall_order_xq set buyer='$td[buyer]',logo='$post[logo]',lm='$post[lm]',server='$post[server]',contact='$post[contact]',pwd='$pwd',others='0',ad='$post[ad]',addtime='$DT_TIME',mallid='$td[mallid]' where itemid=$itemid";
					$db->query($upload_sql);
				$db->query("UPDATE {$table} SET status=2,updatetime=$DT_TIME WHERE itemid=$itemid");//更新订单为2状态
			  	...
			  	while($designs=$db->fetch_array($design_sql)){
					...
					send_msg('index_last',$L[order_index_gj],$rm[email],$rm[username]);;
						$design[]=$designs;
 						...
 						$order_sql=$db->query("select * from d_mall_order_1  group by company_id having level=1 and last=0 and status=0 and checked=1 order by addtime asc limit $malls[scount]");
		...
		case 'pay_1'://买家确认首付款
			...
			if($submit) {
				...
				$db->query("UPDATE {$table} SET status=1,updatetime=$DT_TIME,price_last=price_last+$money WHERE itemid=$itemid");
			...
			case 'pay_2'://买家确认第二次付款
				...
				if($submit) {
					is_payword($_username, $password) or message($L['error_payword']);
					$db->query("UPDATE {$DT_PRE}member SET money=money-$money,locking=locking+$money WHERE username='$_username'");
					$db->query("UPDATE {$table} SET status=5,updatetime=$DT_TIME,price_last=price_last+$money WHERE itemid=$itemid");
				...
				case 'pay_3'://买家确认第三次付款
					...
					if($submit) {
						is_payword($_username, $password) or message($L['error_payword']);
						$db->query("UPDATE {$DT_PRE}member SET money=money-$money,locking=locking+$money WHERE username='$_username'");
						$db->query("UPDATE {$table} SET status=7,updatetime=$DT_TIME,price_last=price_last+$money WHERE itemid=$itemid");
		...
		case 'upload_2'://栏目页上传
		 ...
		 for($i=0;$i<=count($post);$i++){
		 	...
		 	$upload_sql="INSERT INTO d_mall_order_1 (id,rid,company_id,designer,upload,addtime,status,note,level,mallid) VALUES(NULL,$itemid,'$_username','$_company','$thumb','$DT_TIME',0,'$nnote',2,'$td[mallid]')";
	  ...
	  * */
			...
			$order_sql=$db->query("select * from d_mall_order_1  group by company_id having level=1 and last=0 and status=0 and checked=1 order by addtime asc limit $mall[scount]");
       while($order=$db->fetch_array($order_sql)){
       ...
       send_mob('wel_acc',$rm['truename'],$itemid,$rm[mobile]);
		...
		send_mob('wel_acc',$rm['truename'],$itemid,$rm[mobile]);
			...
			if($td['status'] != 7 || $td['buyer'] != $_username || $gone > ($MOD['trade_day']*86400 + $td['add_time']*3600)) message($L['trade_msg_deny']);
			$money = $td['amount'] + $td['fee'];
