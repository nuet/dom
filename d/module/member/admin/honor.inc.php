...
$do = new honor();
$menus = array (
    array('添加作品', '?moduleid='.$moduleid.'&file='.$file.'&action=add'),
    array('作品列表', '?moduleid='.$moduleid.'&file='.$file),
    array('审核作品', '?moduleid='.$moduleid.'&file='.$file.'&action=check'),
    array('过期作品', '?moduleid='.$moduleid.'&file='.$file.'&action=expire'),
    array('未通过作品', '?moduleid='.$moduleid.'&file='.$file.'&action=reject'),
    array('回收站', '?moduleid='.$moduleid.'&file='.$file.'&action=recycle'),
);
if(in_array($action, array('', 'check', 'expire', 'reject', 'recycle'))) {
	$sfields = array('按条件', '作品名称', '发证机构', '会员名');
