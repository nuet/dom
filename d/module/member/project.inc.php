   ...
   $pos=stripos($td[designer], $_company);//判断是否存在   返回bool值
	...
	switch($step) {
		  ...
		  case 'index':
			...
			break;
				...
				case 'design':
					...
					$design_all=$db->get_all("select * from d_mall_order_1 where company_id='$_username'  and level=3");
	  ...
	  * */
			...
			$order_sql=$db->query("select * from d_mall_order_1  group by company_id having level=1 and last=0 and status=0 and checked=1 order by addtime desc limit $mall[scount]");
