...
$item = $db->get_one("SELECT * FROM {$table} A,d_pingmian_data B WHERE A.itemid=B.itemid and A.itemid=$itemid");
if(!$item){
...
if($order_yz) {
   ...
   if($expload[0]=='简体中文' || $expload[0]=='繁体中文' ){
	...
	$sql="insert into d_pingmian_order(mallid,buyer,catid,areaid,hy_sort,seller,title,thumb,price,allow,number,amount,buyer_name,buyer_address,area,buyer_phone,buyer_mobile,buyer_username,buyer_receive,buyer_star,seller_star,addtime,updatetime,status,pay_style,fapiao_yz,mob_yz,language_style) values('$itemid','$_username','$item[catid]','$area[areaname]','$t_sort','$item[username]','$item[title]','$item[thumb]','$item[price]','$allow','1','$last_pirce','$post[name]','$post[address]','$add[areaid]','$post[telephone]','$post[mobile]','$post[truename]','$item[express_name_1]','0','0','$DT_TIME','$DT_TIME','0','$playxz','$fapiaosq','$iswap','$language_style')";
	$db->query($sql);

	
	$id=$db->insert_id();
	$db->query("REPLACE INTO d_pingmian_comment (itemid,mallid,buyer,seller) VALUES ($id,$itemid,'$_username','')");
	$tmp = $db->get_one("SELECT mallid FROM d_pingmian_stat WHERE mallid=$id");
	if(!$tmp) $db->query("REPLACE INTO d_pingmian_stat (mallid,buyer,seller) VALUES ($itemid,'$_username','')");
	
	$db->query("insert into d_pingmian_order_xq (itemid) values('$id')");
