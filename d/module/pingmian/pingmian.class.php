...
class pingmian {
    ...
    function pingmian($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','mycatid','areaid','level','title','style','fee','introduce','brand','price','put','amount','thumb','thumb1','thumb2','tag','status','hits','username','editor','addtime','adddate','edittime','editdate','ip','template','linkurl','filepath','elite','note','company','truename','telephone','mobile','address','email','msn','qq','ali','skype','n1','n2','n3','n4','n5','v1','v2','v3','v4','v5','express_1','express_name_1','last_money','rmoney_1','rmoney_2','rmoney_3','smoney_1','smoney_2','smoney_3','scount','in_select','mobprice','resprice','mlangprice','rlangprice');
    }
	...
	function set($post) {
		...
		$post['mobprice'] = dround($post['mobprice']);
		$post['resprice'] = dround($post['resprice']);
		$post['mlangprice'] = dround($post['mlangprice']);
		$post['rlangprice'] = dround($post['rlangprice']);
		...
		if(strpos($post['v4'], '|') === false) $post['n4'] = $post['v4'] = '';
		if(strpos($post['v5'], '|') === false) $post['n5'] = $post['v5'] = '';
		$post['express_1'] = intval($post['express_1']);
		$post['express_name_1'] = trim($post['express_name_1']);
		$post['last_money'] = dround($post['last_money']);
		$post['rmoney_1'] = dround($post['rmoney_1']);
		$post['rmoney_2'] = dround($post['rmoney_2']);
		$post['rmoney_3'] = dround($post['rmoney_3']);
		$post['smoney_1'] = dround($post['smoney_1']);
		$post['smoney_2'] = dround($post['smoney_2']);
		$post['smoney_3'] = dround($post['smoney_3']);
