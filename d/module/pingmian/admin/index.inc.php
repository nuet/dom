...
switch($action) {
	...
	case 'add':
		if($submit) {
			...
			$EXP = array();
			$result = $db->query("SELECT * FROM {$DT_PRE}pingmian_express WHERE username='$username' AND parentid=0 ORDER BY listorder ASC,itemid ASC LIMIT 100");
		...
		if($itemid && !$psize) {
			...
			if($t=="add"){
				$p=implode(",",$itemid);
					...
					$sql="select * from d_pingmian_order_1 a,d_pingmian_order b where a.rid=b.itemid and a.id in($p) and a. checked=1 group by a.rid";
				...
				$sql="update d_pingmian_order_1 set checked=1 where  id in($p)";
				$db->query($sql);
				dmsg('审核成功', $forward);
			}else if($t=='fall'){
				$p=implode(",",$itemid);
        	$sql="update d_pingmian_order_1 set checked=2 where  id in($p)";
         	$sql="select * from d_pingmian_order_1 a,d_pingmian_order b where a.rid=b.itemid and a.id in($p) and a. checked=1 group by a.rid";
				$order_sql=$db->query($sql);
				while($rs=$db->fetch_array($order_sql)){
					$rm=$db->get_one("select * from d_member where username='$rs[buyer]'");
				...
				$p=implode(",",$itemid);
				$sql="delete from d_pingmian_order_1 where  id in($p)";
				$db->query($sql);
				dmsg('删除作品成功', $forward);
			}
		 ...
		 $table1 = $DT_PRE.'pingmian_order_1';
			$itemid or $itemid = '';
			$pingmianid = isset($pingmianid) && $pingmianid ? intval($pingmianid) : '';
			...
			if($page > 1 && $sum) {
				...
				$r = $db->get_one("select count(*) as num from d_pingmian_order A,d_pingmian_order_1 B where A.itemid=B.rid and B.level=1 AND $condition");
				$items = $r['num'];
			}
			$pages = pages($items, $page, $pagesize);
			$lists = array();
			$result = $db->query("SELECT * FROM d_pingmian_order A,d_pingmian_order_1 B  WHERE A.itemid=B.rid and B.level=1 AND $condition ORDER BY B.addtime desc LIMIT $offset,$pagesize");
			$amount = $fee = $money = 0;
...
// 				}
				...
				if($page > 1 && $sum) {
					...
					$r = $db->get_one("select count(*) as num from d_pingmian_order A,d_pingmian_order_1 B where A.itemid=B.rid and B.level=2 AND $condition");
					$items = $r['num'];
				}
				$pages = pages($items, $page, $pagesize);
				$lists = array();
				$result = $db->query("SELECT * FROM d_pingmian_order A,d_pingmian_order_1 B  WHERE A.itemid=B.rid and B.level=2 AND $condition ORDER BY B.addtime desc LIMIT $offset,$pagesize");
				$amount = $fee = $money = 0;
...
// 					}
			...
			case 'check_sj2':
				if($_POST[pwd]!=""){
					$db->query("update d_pingmian_order_1 set pwd='$_POST[pwd]'");
				}
				if($itemid && !$psize) {
					//通过设计师作品
					if($t=="add"){
					...
					if($page > 1 && $sum) {
						...
						$r = $db->get_one("select count(*) as num from d_pingmian_order A,d_pingmian_order_1 B where A.itemid=B.rid and B.level=3 AND $condition");
						$items = $r['num'];
					}
					$pages = pages($items, $page, $pagesize);
					$lists = array();
					$result = $db->query("SELECT * FROM d_pingmian_order A,d_pingmian_order_1 B  WHERE A.itemid=B.rid and B.level=3 AND $condition ORDER BY B.addtime desc LIMIT $offset,$pagesize");
					$amount = $fee = $money = 0;
