...
$user = $db->get_one("select * from d_member A, d_company B where A.userid=B.userid and A.userid='$_userid'");
		/*设计师  投标入围中标的数量*/
		$sj_all=$db->get_all("select * from d_mall_order_1 where company_id='$_us' group by rid");
		...
		$count1=$db->get_one("select count(*) as count from d_mall_order where buyer='$_username' and status>0");
