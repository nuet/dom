...
function get_cat_parent($catid) {
	global $db;
	$catid = intval($catid);
	return $catid ? $db->get_one("select B.content1 as content1,B.content2 as content2,B.content3 as content3,B.content4 as content4 from d_category A,d_category B where A.parentid=B.catid and A.catid=$catid") : array();
}

function get_cat_parent1($catid) {
	global $db;
	$catid = intval($catid);
	return $catid ? $db->get_one("select * from d_category A,d_category B where A.parentid=B.catid and A.catid=$catid") : array();
}
...
function pages1($total, $page = 1, $perpage = 20, $demo = '', $step = 3) {//大厅
	global $DT_URL, $DT, $L;
	if($total <= $perpage) return '';
	$items = $total;
	$total = ceil($total/$perpage);
	...
	$pages = '';
	include DT_ROOT.'/api/pages.order.php';
	return $pages;
}

function pages2($total, $page = 1, $perpage = 20, $demo = '', $step = 3) {
	global $DT_URL, $DT, $L;
	if($total <= $perpage) return '';
	$items = $total;
	$total = ceil($total/$perpage);
	...
	$pages = '';
	include DT_ROOT.'/api/pages.search.php';
	return $pages;
}
