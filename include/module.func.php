...
function hkmoney_add($username, $amount) {
	global $db;
	if($username && $amount) $db->query("UPDATE {$db->pre}member SET hkmoney=hkmoney+{$amount} WHERE username='$username'");
}
function zkmoney_add($username, $amount) {
	global $db;
	if($username && $amount) $db->query("UPDATE {$db->pre}member SET zkmoney=zkmoney+{$amount} WHERE username='$username'");
}

function yfpmoney_add($username, $amount) {
	global $db;
	if($username && $amount) $db->query("UPDATE {$db->pre}member SET yfpmoney=yfpmoney+{$amount} WHERE username='$username'");
}
