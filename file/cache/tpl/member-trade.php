...
<div class="u_SheJiS_DaTing u_fl">
    ...
    <?php } else if($r['status']==1 && $r['vcheck']==2) { ?>
     ...
     <input type="submit" value="查看栏目页" class="u_btn" onclick="Go('trade.php?itemid=<?php echo $r['itemid'];?>&action=update&step=lm&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
     <?php } else if($r['status']==6 ) { ?>
       <input type="button" value="等待程序制作" class="gaoliangG" />
       <?php } else if($r['status']==7 ) { ?>
         <input type="submit" value="确认验收" class="u_btn" onclick="if(confirm('确认验收么？')) Go('trade.php?itemid=<?php echo $r['itemid'];?>&action=update&step=receive_goods&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
       <?php } else if($r['status']==8 ) { ?>
        <input type="submit" value="付款上线" class="u_btn" onclick="Go('trade.php?itemid=<?php echo $r['itemid'];?>&action=update&step=pay_3&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
