...
<form method="post" action="trades.php" onsubmit="return check();" id="dform">
...
<form method="post" action="trades.php" id="dform">
<input type="hidden" name="forward" value="<?php echo $forward;?>"/>
<input type="hidden" name="action" value="<?php echo $action;?>"/>
<input type="hidden" name="step" value="<?php echo $step;?>"/>
<input type="hidden" name="itemid" value="<?php echo $itemid;?>"/>
...
<tr>
<td class="tl">订单单号</td>
<td class="tr"><a href="trades.php?itemid=<?php echo $itemid;?>&action=update&step=detail">T<?php echo $td['itemid'];?></a></td>
</tr>
<tr>
<td class="tl">产品名称</td>
...
<td><a href="trades.php?itemid=<?php echo $v['rid'];?>&action=update&step=edit_order1&id=<?php echo $v['id'];?>" style="color:#4a4a4a">编辑</a></td>
</tr>
<tr<?php if($k%2==1) { ?> bgcolor="#FAFAFA"<?php } ?>
>
<td colspan="8" class="f_gray">
<span class="f_r">
<strong>单号：</strong><a href="trades.php?itemid=<?php echo $v['rid'];?>&action=update&step=detail" target='_blank'>T<?php echo $v['rid'];?></a>&nbsp;&nbsp;
</td>
</tr>
<?php } } ?>
<?php if($trades) { ?>
...
<form method="post" action="trades.php" name="dform" id="dform">
<input type="hidden" name="forward" value="<?php echo $forward;?>"/>
<input type="hidden" name="action" value="<?php echo $action;?>"/>
<input type="hidden" name="step" value="<?php echo $step;?>"/>
<input type="hidden" name="itemid" value="<?php echo $itemid;?>"/>
...
<tr>
<td class="tl">当前操作</td>
<td class="tr f_red f_b">
<?php if($td['designer']==$_company) { ?>
对客户进行评价
<?php } else { ?>
对设计师户评价
<?php } ?>
...
<div class="u_SheJiS_DaTing u_fl">
              ...
              <p>项目搜索</p>
               <form action="trades.php">
                 ...
                 <span class="u_SheJiS_DaTing_JG_sea_txt1"> <?php echo dcalendar('fromtime', $fromtime);?> <i>至</i> <?php echo dcalendar('totime', $totime);?></span>
                  <input type="submit" value="搜素" name="submit" class="u_SheJiS_DaTing_JG_sea_btn"><a href="trades.php" style='float:right;height:38px;line-height:38px'>重置筛选条件</a>
                 </form>
            </div>
           ...
           <?php if(is_array($trades)) { foreach($trades as $i => $r) { ?>
            ...
            <div class="u_SheJiS_DaTing_List_cont">
              ...
              <div class="u_SheJiS_DaTing_List_cont_xmzt">
                    ...
                    <p><a href="trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=logo&forward=<?php echo $MODULE['2']['linkurl'];?>">订单详情</a></p>
              </div>
    ...
    <input type="submit" value="立即付款" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=pay&forward=<?php echo $MODULE['2']['linkurl'];?>');">
    <?php } else if($r['pay_style']==1 && $r['status']==0) { ?>
     <input type="submit" value="首付款操作" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=pay_1&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
     <?php } else if($r['status']==1 && $r['vcheck']==0) { ?>
     <input type="submit" value="提交需求" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=logo&forward=<?php echo $MODULE['2']['linkurl'];?>');"/> 
    ...
    <?php } else if($r['status']==1 && $r['vcheck']==2) { ?>
     ...
     <input type="submit" value="入围作品" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=logo_index&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
     
      <?php } else if($r['status']==3 ) { ?>
    
     <input type="submit" value="确定首页" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=logo_index&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
       <?php } else if($r['status']==4 ) { ?>
     <input type="submit" value="二次付款" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=pay_2&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
      <?php } else if($r['status']==5 ) { ?>
     <input type="submit" value="选择作品" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=lm&forward=<?php echo $MODULE['2']['linkurl'];?>');"/>
    <?php } else if($r['status']==6 ) { ?>
     <input type="submit" value="程序开发" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=design&forward=<?php echo $forward;?>');"/>
     <?php } else if($r['status']==7 ) { ?>
     <input type="submit" value="确认完成" class="u_btn" onclick="if(confirm('确认项目已制作完成么？\n\n请注意:确认后您的钱将直接支付给卖家')) Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=receive_goods&forward=<?php echo $forward;?>');"/>
    <?php } else if($r['status']==8 ) { ?>
    <?php if($r['seller_star']) { ?>
<input type="submit" value="评价详情" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=comment_detail&forward=<?php echo $forward;?>');"/>
<?php } else { ?>
<input type="submit" value="评 价" class="u_btn" onclick="Go('trades.php?itemid=<?php echo $r['itemid'];?>&action=update&step=comment&forward=<?php echo $forward;?>');"/>
<?php } ?>
     <?php } else { ?>
