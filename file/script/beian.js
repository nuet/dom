...
$.extend($.BeianObj,{
	...
	Title:{
				...
				Zjhm:{msg:"证件号码",width:200,height:20,left:25,fn:function(Obj){
					 
					 if($('#Zjlx option:selected').val()==1){
						 var idcard = $.trim($("input[name='Zjhm']:text").val());
						  ...
						  if (parseInt(idcard.substr(6, 4)) % 4 == 0 || (parseInt(idcard.substr(6, 4)) % 100 == 0 && parseInt(idcard.substr(6, 4)) % 4 == 0)) {
			                ...
			                if (reg.test(idcard)) {
			                    ...
			                    if (M == idcard_array[17]) {
			                    	...
			                    	$.BeianObj.PutOver("load",Obj);	
			                    	//return {node:obj,result:false,msg:'身份证号码错误，请核对并修改'};
			                    	$.BeianObj.PutOver("err",Obj,"身份证号码错误，请核对并修改!");
			                    }
			                } else {$.BeianObj.PutOver("load",Obj);	
			                    //return {node:obj,result:false,msg:'身份证号码出生日期有误，请按照证件中的出生日期填写'};
			                	$.BeianObj.PutOver("err",Obj,"身份证号码出生日期有误，请按照证件中的出生日期填写!")
			                }
						  
						 
					 }else{
						 if(!/^(P|p\d{7})|(G|g\d{8})$/.test($(Obj).val())){	
								$.BeianObj.PutOver("load",Obj);						
								$.BeianObj.PutOver("err",Obj,"护照类型格式错误!");
								return;	
						  }else{
							   $.BeianObj.CheckInput(Obj);
						  }
				...
				ztFzr_zjhm:{msg:"企业法人证件号码",width:200,height:20,left:25,fn:function(Obj){
								 
								 if($('#ztFzr_zjlx option:selected').val()==1){
									 var idcard = $.trim($("input[name='ztFzr_zjhm']:text").val());
				...
				mobilecode:{msg:"请输入发送到您手机的验证码(不区分大小写)<br><font color=red>若3分钟内还未收到，请再次\"获取手机验证码\",系统将自动换通道发送</font>.验证码5小时内均有效,如果发三次仍收不到的，请用您的手机拔打021-51875581由客服人工提供验证码。切勿借他人手机来获取验证码，因为管局还会打手机进行真实性核验！",width:300,height:100,left:25,fn:function(Obj){if(!/^\w{4,6}$/.test($(Obj).val())){	
								$.BeianObj.PutOver("load",Obj);						
				...
				Fzr_zjlx:{msg:"负责人证件类型",width:200,height:30,left:25,fn:function(Obj){
								 if(!/^\d{1,2}$/.test($(Obj).val())){	
										$.BeianObj.PutOver("load",Obj);						
										$.BeianObj.PutOver("err",Obj,"证件类型格式错误!");
										return;	
				...
				Fzr_zjhm:{msg:"负责人证件号码",width:200,height:20,left:25,fn:function(Obj){
							 
							 if($('#Fzr_zjlx option:selected').val()==1){
								 var idcard = $.trim($("input[name='Fzr_zjhm']:text").val());
...
//					
					...
					$("input[name='submitbuttonwz']:button").click(function(){
						...
						if($(this).attr("alt")=="nextadd"){
							$.BeianObj.DoSubmit("new_add");
						}else{
							$.BeianObj.DoSubmit("add");
						}
					});	
					...
					if(submitbuttonfile.get(0)){
						submitbuttonfile.click(function(){
							if($.BeianObj.Settings.ischeckuser){
								var isfindwzzp=true;
								$("img[id^='showthumb']").each(function(index, fzrzpObj) {
									console.log(fzrzpObj);
		                            if($.trim($(fzrzpObj).attr("src"))=="http://www.d.com/skin/default/image/waitpic.gif"){																			
										isfindwzzp=false;
...
function sendSms(){	
	...
	var info="act=sendsms&mobile="+ escape(titlemsg.val())+"&Fzr_xm="+Fzr_xmObj.val();
