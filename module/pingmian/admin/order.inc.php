...
require MD_ROOT.'/pingmian.class.php';
...
$menus = array (
    ...
    array('审核设计师首页','?moduleid=100&action=check_sj'),
    array('审核设计师PSD','?moduleid=100&action=check_sj2'),
...
$_status= array(
		...
		'<span style="color:#008000;">买家已付款<br/>等待网站制作最终页</span>',
		'<span style="color:#FF0000;text-decoration:underline;">网站制作完成<br>等待确认</span>',
...
$table1 = $DT_PRE.'pingmian_order_1';
if($action == 'refund' || $action == 'show' || $action == 'comment') {
    ...
    $item_doc=$db->get_one("select * from d_pingmian_order_xq where itemid=$itemid");
     $zl=$db->get_all("select * from d_pingmian_zl where itemid='$itemid'");

    $pic=$db->get_one("select * from d_pingmian_order_1 where id='$id'");
...
switch($action) {
	...
	case 'show':
		...
		$cn = $db->get_one("SELECT * FROM d_pingmian_order_website WHERE itemid=$itemid");
	...
	case 'shenhe':
		...
		if($_GET[shenhe]==2){
			...
			$db->query("update  d_pingmian_order_xq set vcheck='$_GET[shenhe]' where itemid='$itemid'");
			...
			$db->query("update  d_pingmian_order_xq set vcheck='$_GET[shenhe]',addtime='' where itemid='$itemid'");
			$rm[msg]=$_GET[msg];
			//- send email start-//
			$title =$L[ziliao_shenhe];
		...
		break;
	case 'edit':
		$cm = $db->get_one("SELECT * FROM d_pingmian_order_website WHERE itemid=$itemid");
		$order=$db->get_one("SELECT * FROM d_pingmian_order WHERE itemid=$itemid");
		if($cm){
			$action='edit';
		}else{
			$action='add';
		...
		if($submit) {
			if($num!=""){
				$db->query("update d_pingmian_order set url='$num' where itemid='$itemid'");
			}
			$db->query("update d_pingmian_order_website set username='$username',password='$password',url='$url',adminurl='adminurl',note='$note',num='$num' where itemid=$itemid");
			dmsg('编辑成功', $forward);
		}		
	...
	case 'comment':
		...
		if($cm['seller_star'] != $post['seller_star']) {
			...
			$db->query("UPDATE {$DT_PRE}pingmian_order SET seller_star=$s WHERE itemid=$itemid");
			$db->query("UPDATE {$DT_PRE}pingmian_stat SET `$s2`=`$s2`+1 WHERE mallid=$mallid");
			if($cm['seller_star']) $db->query("UPDATE {$DT_PRE}pingmian_stat SET `$s1`=`$s1`-1 WHERE mallid=$mallid");
		}
		if($cm['buyer_star'] != $post['buyer_star']) {
			$s = $post['buyer_star'];
			$s1 = 'b'.$cm['buyer_star'];
			$s2 = 'b'.$post['buyer_star'];
			$db->query("UPDATE {$DT_PRE}pingmian_order SET buyer_star=$s WHERE itemid=$itemid");
			$db->query("UPDATE {$DT_PRE}pingmian_stat SET `$s2`=`$s2`+1 WHERE mallid=$mallid");
			if($cm['buyer_star']) $db->query("UPDATE {$DT_PRE}pingmian_stat SET `$s1`=`$s1`-1 WHERE mallid=$mallid");
		}
		$sql = '';
		foreach($post as $k=>$v) {
			$sql .= ",$k='$v'";
        ...
        $sql = substr($sql, 1);
	    $db->query("UPDATE {$DT_PRE}pingmian_comment SET $sql WHERE itemid=$itemid");
	...
	default:
		...
		$person=$db->get_one("select * from d_member where username='$_username'");
		if($action=='add'){
			if($num!=""){
				$db->query("update d_pingmian_order set url='$num' where itemid='$itemid'");
			}
			$db->query("insert into d_pingmian_order_website (itemid,username,password,url,adminurl,note,num) values('$itemid','$username','$password','$url','$adminurl','$note','$num')");
			dmsg('增加成功', $forward);
		}
