...
$weihus=$db->get_all("SELECT * FROM {$DT_PRE}weihu_order WHERE buyer='$_username' and status=4 and endtime>'$DT_TIME'");

if($itemid && $t==1){
	$weihu=$db->get_one("SELECT * FROM {$DT_PRE}weihu_order WHERE itemid='$itemid' and buyer='$_username' and status=4 and endtime>'$DT_TIME'");
	if($weihu){
		echo json_encode($weihu);
	}else{
		echo "-1";
	}
	exit;
...
switch($action) {
	case 'add':
		if($submit) {
			...
			$lb=explode("-",$lb);
		 	if(!$lb || !$whid){
				message('已过期或不存在此域名的订单信息');
				exit;
			}else{
				$xz=$db->get_one("SELECT * FROM {$DT_PRE}weihu_order WHERE itemid='$whid' and buyer='$_username' and status=4 and endtime>'$DT_TIME'");
				if($xz==""){
				message('已过期或不存在此域名的订单信息');	
				}
