...
if($submit) {
	...
	if ($_username && $_groupid <=5){
		...
		while($v = $db->fetch_array($rn)) {
			...
			$v[status_zt]=$L['logo_status'][$v[status]];
		...
		$count3=$db->get_one("select count(*) as count from d_mall_order where buyer='$_username' and status=9");
		...
		while($v = $db->fetch_array($rn)) {
			...
			$v[status_zt]=$L['logo_trade_status'][$v[status]];
