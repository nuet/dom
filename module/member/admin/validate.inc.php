...
$menus = array (
    array('设计师认证', '?moduleid='.$moduleid.'&file='.$file.'&action=company'),
...
$V = array('company'=>'设计师认证', 'truename'=>'实名认证', 'mobile'=>'手机认证', 'email'=>'邮件认证');
	...
	case 'cancel':
		...
		foreach($itemid as $id) {
			...
			if($r) {
				...
				$db->query("UPDATE {$DT_PRE}member SET `{$vfd}`=0 WHERE username='$username'");
				...
				if($msg) {
					$t = $db->get_one("SELECT mobile FROM {$DT_PRE}member WHERE username='$username'");
					if($t) {
						$content = '您的'.$V[$fd].'已经被取消，请重新认证';
						if($reason) $content .= ',取消原因:'.$reason;
						send_sms($t['mobile'], $content.$DT['sms_sign']);
					}
				}
				$i++;
			}
