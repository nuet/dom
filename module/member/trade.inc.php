...
if($action == 'update') {
	...
	$mall_sql="select * from d_mall where itemid='{$td[mallid]}'";
	...
	switch($step) {
		case 'upload_1'://设计师上传第一次作品
		    ...
		    case 'logo':
		    	...
		    	if($_POST[confirm_order]==2){
		    		if($zl_count[count]>5){
		    			...
		    			$db->query("insert into d_mall_zl (itemid,zl,addtime) values('$td[itemid]','$_POST[zl]','$DT_TIME')");
		    			message("提交成功",$forward);
		    		}
		    			
		    	}else if($_POST[confirm_order]==1)
		    	{
		    		$db->query("update d_mall_order_xq set note='$_POST[note]' where itemid=$itemid");
			...
			case 'upload_5'://买家上传需求
				...
				case 'xq'://买家编辑需求
					...
					if($submit) {
						...
						$upload_sql="update  d_mall_order_xq  set  updatetime='$DT_TIME',others='$post[others]' where itemid=$itemid";
			 ...
			 * */
				...
				$last[upload]=str_replace("thumb","middle",$last[upload]);
				...
				while($re_upload_1=$db->fetch_array($tag_sql)){
					$sql="select * from d_mall_order_1  where  checked=1 and level=1 and rid='$itemid' and company_id='$re_upload_1[company_id]'";
					$all_sql=$db->query($sql);
					while ($all=$db->fetch_array($all_sql)) {
						$all[addtime]=date("Y-m-d H:i:s",$all[addtime]);
						$all[upload]=str_replace("thumb","middle",$all[upload]);
						$re_upload_1[t][]=$all;
					}
					...
					$wall[addtime]=date("Y-m-d H:i:s",$wall[addtime]);
						$wall[upload]=str_replace("thumb","middle",	$wall[upload]);
					 $index_alls[]=$wall;
  				...
  				if($submit) {
					...
					$db->query("update d_mall_order_1 set status=1  where id in($up)");
					/*
					 * By-摩恩科技
					...
					* 中文语言包在/lang/zh-cn/member.inc.php中
					* */
						$design_sql=$db->query("select * from d_mall_order_1 where rid='$itemid' and status=1 and level=1");
						...
						$rm=$db->get_one("select * from d_member where username='$designs[company_id]'");
						   $url = $CFG[url].'member/trade.php?itemid='.$designs[rid];
					...
					if(count($post)>1){
				      ...
				      $db->query("update d_mall_order set designer='$mm',in_index='$mm' where itemid=$itemid");
			          ...
			          message($L['trade_confirm_index_success'], $forward, 3);
  						...
  						$db->query("update d_mall_order_1 set last=1 where level=1 and id in($up)");
  						
  					
  						$design_sql=$db->query("select * from d_mall_order_1 where rid='$itemid' and status=1 and last=1 and level=1");
  						while($designs=$db->fetch_array($design_sql)){
  							$db->query("update d_mall_order set thumb='$designs[upload]' where itemid='$designs[rid]'");
  							$rm=$db->get_one("select * from d_member where username='$designs[company_id]'");
  							$url = $CFG[url].'member/trade.php?itemid='.$designs[rid];
  						...
  						$order_sql=$db->query("select * from d_mall_order_1  group by company_id having level=1 and last=0 and status=0 and checked=1 order by addtime desc limit $malls[scount]");
  						...
  						$db->query("update d_mall_order set designer='$mm' where itemid=$itemid");
			    ...
			    case 'upload_4':
		 		...
		 		case 're_upload_2':
		 			...
		 			*/
		 			...
		 			//echo "select * from d_mall_order_1 where rid=$itemid";
			   ...
			   while($re_upload_1=$db->fetch_array($tag_sql)){
				...
				case 'last':
					...
					case 'lm':
						...
						$product=$db->get_one("select * from d_mall where itemid='$td[mallid]'");
						$tag_sql=$db->query("select * from d_mall_order_1  group by designer,rid having rid='$itemid'  ");
		             
						while($re_upload_1=$db->fetch_array($tag_sql)){
							$sql="select * from d_mall_order_1  where  checked=1 and level=2 and rid='$itemid' and company_id='$re_upload_1[company_id]'";
				    	...
				    	send_msg('lm_last',$L[order_index_tz],$rm[email],$rm[username]);
							...
							$db->query("update d_mall_order_1 set last=1 where level=2 and id in($up)");
		...
		case 'pay_1'://买家确认首付款
			...
			$t=$db->get_one("select * from d_mall where itemid=$td[mallid]");
			...
			case 'pay_2'://买家确认第二次付款
				...
				break;
				case 'pay_3'://买家第三次付款并上线
					if($td['status'] !=8 || $td['buyer'] != $_username) message($L['trade_msg_deny']);
					$t=$db->get_one("select * from d_mall where itemid=$td[mallid]");
					//$money = $td['amount'] ;
					$money=$td[amount]*0.2;
					$seller = userinfo($td['seller']);
					...
					if($submit) {
				  	 ...
				  	 $endtime=$endtime;
				$db->query("UPDATE {$table} SET status=9,updatetime=$DT_TIME,ready_money=ready_money+$money,endtime='$endtime' WHERE itemid=$itemid");
...
//			
//	
//			$mall=$db->get_one("select * from d_mall where itemid=$td[mallid]");
//		
//			function grade($t){
//				global $db;
...
//				}
//		
//			$order_sql=$db->query("select * from d_mall_order_1  group by company_id having level=1  and status=0 and checked=1 order by addtime desc limit $mall[scount]");
//       while($order=$db->fetch_array($order_sql)){
//       	
//       	$rm=$db->get_one("select * from d_member where username='$order[company_id]'");
...
/*/*		   money_add($td['seller'], $money-$money1);
		   ...
		   $endtime=time()+(365 * 24 * 60 * 60);
		   $endtime=$endtime;
			$db->query("UPDATE {$table} SET status=8,updatetime=$DT_TIME,endtime='$endtime' WHERE itemid=$itemid");
			...
			message($L['trade_success'], $forward, 3);//交易成功
*/		break;
          case 'receive_goods'://确认验收
          	if($td['status'] != 7 || $td['buyer'] != $_username)  message($L['trade_msg_deny']) ;
          	if($td[pay]==0){
          	$money = $td['amount'] + $td['fee'];
          	money_lock($_username, -$money);		
          money_record($_username, -$money, $L['in_site'], 'system', $L['trade_record_pay'], $L['trade_order_id'].$itemid);
		   ...
		   $endtime=time()+(365 * 24 * 60 * 60);
		   $endtime=$endtime;
			$db->query("UPDATE {$table} SET status=9,updatetime=$DT_TIME,endtime='$endtime' WHERE itemid=$itemid");
			$db->query("UPDATE {$DT_PRE}mall SET orders=orders+1,sales=sales+$td[number],amount=amount-$td[number] WHERE itemid=$mallid");
			$touser = $td['seller'];
			$title = lang($L['trade_message_t4'], array($itemid));
			$url = $memberurl.'trade.php?itemid='.$itemid;
		...
		case 'comment'://交易评价
			...
			if($_company == $td['designer']) {
				...
				if($submit) {
					...
					$s = 'b'.$star;
					$db->query("UPDATE {$DT_PRE}mall_comment SET buyer_star=$star,designer='$td[designer]',buyer_comment='$content',buyer_ctime=$DT_TIME WHERE itemid=$itemid");
				...
				if($submit) {
					...
					$s = 's'.$star;
					$tmp = $db->get_one("SELECT mallid FROM d_mall_stat_sjs WHERE designer='$td[designer]'");
					if(!$tmp) {
						$db->query("REPLACE INTO d_mall_stat_sjs (mallid,buyer,designer) VALUES ($itemid,'$_username','$td[designer]')");
						$db->query("UPDATE {$DT_PRE}mall_stat_sjs SET scomment=scomment+1,`$s`=`$s`+1 WHERE designer='$td[designer]'");
					}else{
						$db->query("UPDATE {$DT_PRE}mall_stat_sjs SET scomment=scomment+1,`$s`=`$s`+1 WHERE designer='$td[designer]'");
					}
					$db->query("UPDATE {$DT_PRE}mall_comment SET seller_star=$star,designer='$td[designer]',seller_comment='$content',seller_ctime=$DT_TIME WHERE itemid=$itemid");
...
if($step=='pay' || $step=='pay_1' || $step=='pay_2'){
	...
	if($xq[vcheck]!=0 || $td[status]!=1){
		include template('view_4', $module);
	}else{
	...
	include template('view_0', $module);
}else if($step=="lm"){
	include template('view_1', $module);
}else if($step=="logo"){
	if($xq[vcheck]!=0 || $td[status]!=1){
	...
	include template('view_2', $module);
}else if($step=="last"){
	include template('view_3', $module);
}else {
	include template('trade', $module);
}
