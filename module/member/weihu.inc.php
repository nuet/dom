...
if($action == 'update') {
	...
	$td['linkurl'] = $EXT['linkurl'].'redirect.php?mid=99&itemid='.$td['mallid'];
	$mallid = $td['mallid'];
	...
	switch($step) {
		case 'edit_price'://修改价格||确认订单
			...
			if($submit) {
				...
				if(isset($confirm_order)) {
					...
					$url = $memberurl.'weihu.php?action=order&itemid='.$itemid;
		...
		case 'pay'://买家付款
			...
			if($submit) {
				...
				$url = $memberurl.'weihu.php?itemid='.$itemid;
		...
		case 'receive_goods'://确认收货
			...
			$endtime=$td[number]*$endtime;
			$db->query("UPDATE {$table} SET status=3,updatetime=$DT_TIME,endtime='$endtime' WHERE itemid=$itemid");
			//更新商品数据
	...
	while($r = $db->fetch_array($result)) {
		...
		$r['linkurl'] = $EXT['linkurl'].'redirect.php?mid=99&itemid='.$r['mallid'];
