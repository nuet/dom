	...
	if($post) {
		...
		foreach($post as $k=>$v) {
			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				$db->query("INSERT INTO {$DT_PRE}weihu_order (mallid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note,buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,buyer_receive,status,fee_name,fee,dmhf,mmcz,jscl,htsh,ymwh,sjwh,wzlr,cplr) VALUES ('$itemid','$_username','$t[username]','$title','$t[thumb]','$t[price]','$number','$amount','$DT_TIME','$DT_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile','$buyer_receive','$status','$fee_name','$fee','$t[dmhf]','$t[mmcz]','$t[jscl]','$t[htsh]','$t[ymwh]','$t[sjwh]','$t[wzlr]','$t[cplr]')");
				$itemid = $db->insert_id();
