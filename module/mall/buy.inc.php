	...
	if($post) {
		...
		foreach($post as $k=>$v) {
			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				$endtime=$endtime;
				$db->query("INSERT INTO {$DT_PRE}mall_order (mallid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,buyer_receive,status,fee_name,fee,endtime) VALUES ('$itemid','$_username','$t[username]','$title','$t[thumb]','$t[price]','$number','$amount','$DT_TIME','$DT_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile','$buyer_receive','$status','$fee_name','$fee',$endtime)");
				$itemid = $db->insert_id();
