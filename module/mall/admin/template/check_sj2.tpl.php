...
if($v[updatetime]=="0"){
	...
	?>
<?php  $v[pic_upload]=pic_add($v[upload]);?>
<tr onmouseover="this.className='on';" onmouseout="this.className='';" align="center">
<td><input type="checkbox" name="itemid[]" value="<?php echo $v['id'];?>"/></td>
<td class="px11"><a style="color:blue;" href="javascript:Dwidget('?moduleid=16&amp;file=order&amp;action=show&amp;mallid=&amp;itemid=<?php echo $v[rid] ?>', '[<?php echo $v['rid'];?>] 订单列表');"><img src="<?php echo $CFG[url] ?>member/image/<?php echo $v[pic_upload] ?>.ico"  onerror="nofind();"width='60px' height='60px'/></a></td>
<td class="px11"><?php echo $v['note'];?></td>
...
<?php if($action == 'check_sj2') { ?>

<input type="submit" value=" 通过审核 " class="btn" onclick="this.form.action='?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=check_sj2&t=add';"/>&nbsp;
<input type="submit" value=" 拒 绝 " class="btn" onclick="this.form.action='?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=check_sj2&t=fall';"/>&nbsp;
<input type="submit" value=" 彻底删除 " class="btn" onclick="if(confirm('确定要删除选中商品吗？此操作将不可撤销')){this.form.action='?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=check_sj2&t=del'}else{return false;}"/>&nbsp;
<?php } ?>
</div>
</form>
<form method="post" onsubmit="return check()">
一键更新所有文件密码：<input type="text" name='pwd'id='pwd' /><input type="submit" value="确定">
</form>
