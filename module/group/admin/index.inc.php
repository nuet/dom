...
switch($action) {
		...
		if($submit) {
			...
			if($do->pass($post)) {
			    ...
			    if($post[ys]==2){
         	     ...
         	     $upload_sql="INSERT INTO d_group_website (id,itemid,username,url,checked) VALUES(NULL,$do->itemid,'$_username','$post[url]',1)"; 		
			     $db->query($upload_sql);	
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				if($post[ys]==2){
		       	...
		       	$db->query("update d_group_website set url='$post[url]' where itemid='$itemid'");	
				}
				dmsg('修改成功', $forward);
			} else {
				msg($do->errmsg);
	...
	break;
	case 'deletes':
		$db->query("delete from d_group_website where id='$p'");
		echo "1";
		exit;
	break;
	case 'restore':
