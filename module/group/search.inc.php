...
if($_GET[action]=='catids' || $_POST[action]=='catids'){
	//$category_select=ajax_category_select('catid', $L['all_category'], $ppp, $moduleid);
...
if($DT_QST) {
	...
	if($color) $condition .= " AND color='$_GET[color]'";
	if($item) $condition .= " AND title like '%$_GET[item]%'";
...
$all_types=$db->get_all("select * from d_category where moduleid=17 and parentid=0");
$showpage = 1;
$datetype = 5;
$width = 250;
$height = 180;
